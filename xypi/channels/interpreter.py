...
from xypi.spatial.space_config import grid_layout_dict, grid_shape
...
def _interpret_points_0(config: ChannelConfig, geometry: BaseGeometry) -> Channel:
    ...
    for step in range(n_steps):
        ...
        if candidates:
            px, py, pitch_coord, col, row = candidates[0]
            inside = True
            hit = bool(pattern[step])
            value = _event_value(px, py, pitch_coord, config, bounds) if hit else 0.0
